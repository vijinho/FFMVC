...
trait ControllerSecurity
{
     ...
     */
    public function checkCSRF(string $url = '@index', array $params = []): bool
    {
        $f3 = \Base::instance();
     ...
     * @return bool
     */
    public function ipCheck(): bool
    {
        $f3 = \Base::instance();
        $cache = \Cache::instance();
        $ip = $f3->get('IP');
        $f3->set('DNSBL', $f3->get('app.dnsbl'));
        if (!$cache->exists($ip, $isBlacklisted)) {
            $isBlacklisted = $f3->blacklisted($ip);
            $cache->set($ip, $isBlacklisted, $f3->get('app.ttl_blacklist'));
        }
